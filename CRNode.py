...
class CRNode:
  ...
  # Constructor initializes a node with the Meta tags Type, Name, Description, 
  # and Help.
  def __init__(self, Crabs, TypeID = 0, Type = '', Command = None, Cursor = 0):
    ...
    self.Functions = {
      ...
      '#':    CRNode.ListMeta,
      '!':    CRNode.ListDetails,
      'help': CRNode.PrintHelp,
    } 
    Crabs.Push(self, Command, Cursor)
  ...
  def PushPop(self, Node, Command = None, Cursor = 0):
    return Crabs.PushPop(self.Parent, Command, Cursor)
  ...
  # Prints the most important information to the String.
  ...
  def ListMeta(self, WikiSpy, Command = None, Cursor = 0):
    Result = ''
  ...
  # Returns a string with all of the Keys in the Meta and Children indented to the PathDepth().
  def ListChildren(self, WikiSpy, Command = None, Cursor = 0):
    Index = 0
    Result = ''
    for Key, Value in self.Children.items():
      Index += 1
  ...
  # Returns a string with all of the Keys in the Meta and Children indented to the PathDepth().
  def List(self, WikiSpy, Command = None, Cursor = 0):
    Depth = self.PathDepth ()
    String = Stringf.Indent(Depth, '> ' + self.Key())
    String += self.ListChildren(WikiSpy, Command, Cursor)
    String += Stringf.Indent(Depth + 1, '> Meta')
    String += self.ListMeta (WikiSpy, Command, Cursor)
    String += Stringf.Indent(Depth + 1, '<')
    String += Stringf.Indent(Depth, '<')
    return String
  
  # Returns a string with all of the Keys in the Meta and Children indented to the PathDepth().
  def ListDetails(self, WikiSpy, Command = None, Cursor = 0):
     self.List(WikiSpy, Command, Cursor)
  ...
  # Triggers all of the children to Trigger!
  def Trigger(self, Crabs, Result):
    for Child in self.Children:
      Child.Trigger(Result)
    return Result
